...
//Режим работы, закометируйте строку ниже если хотите соединится с существующей сетью
#define useAP  // Если определено то работает как точка доступа если нет то цепляется к вайфай сети

// Для  доступа задаются параметры точки доступа, если соединение к существующей сети WiFi то параметры этой сети
...
static esp_err_t capture_handler(httpd_req_t *req) {
  ...
  fmt2jpg(resized_matrix->item, out_len, EI_CLASSIFIER_INPUT_WIDTH, EI_CLASSIFIER_INPUT_HEIGHT, PIXFORMAT_RGB888, 30, &_jpg_buf, &out_len);//10 было
